...
----------------#lag function-------------------
select Loan_ID,customer_id,Loanamount, Lag(Loanamount) over (order by Loanamount desc) as previousLoanamount from Loans;

--------------------#leadfunction-------------------
select Loan_ID,customer_id,Loanamount, lead(Loanamount) over (order by Loanamount desc) as previousLoanamount from Loans;

select * from Customers;
select * from Accounts;

select firstname, surname, email, Balance, AccountType from Customers join Accounts on Customers.customer_id = Accounts.customer_id;
...
id int,
email varchar(30));
insert into Demo1(id,name) values (1,'Prachi'), (2,'Tejali'), (4,'Sanajana'), (5,'Anjali'), (6,'Ankita');
insert into Demo2(id,email) values (1,'prachi@gmail.com'), (2,'tejali@gmail.com'), (3,'tejali@gmail.com'), (7,'raghav@gmail.com'), (8,'priay@gamil.com');
...
select *from Transactions;
update Accounts
set Balance='15000' where AccountID =204;
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID=(select AccountID from Accounts where Balance=30000); 
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID>(select AccountID from Accounts where AccountID=201);
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID<(select AccountID from Accounts where AccountID=206);
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID>=(select AccountID from Accounts where AccountID=201);
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID<=(select AccountID from Accounts where AccountID=201);
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID <>(select AccountID from Accounts where AccountID=201);

Set @AccountID=201;
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID=(select AccountID from Accounts where AccountID=@AccountID);
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID=(select Max(AccountID) from Accounts);
select Transaction_Date, Amount, Transaction_Type from Transactions where AccountID=(select min(AccountID) from Accounts);
select customer_id, Balance, AccountType from Accounts where Balance=(select Max(Balance) from Accounts);
select customer_id, Balance, AccountType from Accounts where Balance=(select min(Balance) from Accounts);

-------------#>any----------------------
select * from Customers;
select * from Accounts;
select customer_id, firstname, surname from Customers where customer_id<any(select customer_id from Accounts where AccountType='Savings');

select customer_id, firstname, surname from Customers where customer_id>any(select customer_id from Accounts where AccountType='Savings');

-----------#all-----------------
select customer_id, firstname, surname from Customers where customer_id >all(select customer_id from Accounts where AccountType='Savings');
select customer_id, firstname, surname from Customers where customer_id<all(select customer_id from Accounts where AccountType='Savings');
